...
describe("EasySwap Test", function () {
    ...
    describe("should initialize successfully", async () => {
        ...
        it("should expose tokenURI for test erc721", async () => {
            const uri = await testERC721.tokenURI(0)
            expect(uri).to.equal("ipfs://test-erc721/0")

            await testERC721["setTokenURI(string)"]("ipfs://collection/")
            const uri2 = await testERC721.tokenURI(1)
            expect(uri2).to.equal("ipfs://collection/1")

            await testERC721["setTokenURI(uint256,string)"](1, "ipfs://custom/1.json")
            const uri3 = await testERC721.tokenURI(1)
            expect(uri3).to.equal("ipfs://custom/1.json")
        })
    })
    ...
    describe("should multicall successfully", async () => {
        it("should batch make and cancel order in one tx", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const order = {
                side: Side.List,
                saleKind: SaleKind.FixedPriceForItem,
                maker: owner.address,
                nft: [0, testERC721.address, 1],
                price: toBn("0.01"),
                expiry: now,
                salt: 1234,
            }
            ...
            const makeData = esDex.interface.encodeFunctionData("makeOrders", [[order]])
            const cancelData = esDex.interface.encodeFunctionData("cancelOrders", [[orderHash]])

            const tx = await esDex.multicall([makeData, cancelData], true)
            await tx.wait()
        ...
        it("should continue when inner call fails if revertOnFail is false", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const listOrder = {
                side: Side.List,
                saleKind: SaleKind.FixedPriceForItem,
                maker: owner.address,
                nft: [1, testERC721.address, 1],
                price: toBn("0.01"),
                expiry: now,
                salt: 2234,
            }
            await esDex.makeOrders([listOrder])
            const listOrderHash = await testLibOrder.getOrderHash(listOrder)
            ...
            const badMatchData = esDex.interface.encodeFunctionData("matchOrder", [badSellOrder, badBuyOrder])
            const cancelData = esDex.interface.encodeFunctionData("cancelOrders", [[listOrderHash]])

            const [successes] = await esDex.callStatic.multicall([badMatchData, cancelData], false)
            expect(successes[0]).to.equal(false)
            expect(successes[1]).to.equal(true)

            await esDex.multicall([badMatchData, cancelData], false)
            expect(await esDex.filledAmount(listOrderHash)).to.equal(Uint256Max)
            expect(await testERC721.ownerOf(1)).to.equal(owner.address)
        })

        it("should revert whole tx when inner call fails if revertOnFail is true", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const listOrder = {
                side: Side.List,
                saleKind: SaleKind.FixedPriceForItem,
            ...
            await expect(esDex.multicall([badMatchData, cancelData], true))
                .to.be.revertedWith("HD: same maker")

            expect(await esDex.filledAmount(listOrderHash)).to.equal(0)
            expect(await testERC721.ownerOf(2)).to.equal(esVault.address)
        })

        it("should revert on unsupported selector", async () => {
            const pauseData = esDex.interface.encodeFunctionData("pause", [])
            await expect(esDex.multicall([pauseData], true))
                .to.be.revertedWith("HD: unsupported selector")
        })

        it("should revert when batching more than one value-sensitive call", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const order1 = {
                side: Side.List,
                saleKind: SaleKind.FixedPriceForItem,
                maker: owner.address,
                nft: [4, testERC721.address, 1],
                price: toBn("0.01"),
                expiry: now,
                salt: 4234,
            }
            const order2 = {
                ...
                saleKind: SaleKind.FixedPriceForItem,
                maker: owner.address,
                nft: [5, testERC721.address, 1],
                price: toBn("0.01"),
                expiry: now,
                salt: 4235,
            }
            const makeData1 = esDex.interface.encodeFunctionData("makeOrders", [[order1]])
            const makeData2 = esDex.interface.encodeFunctionData("makeOrders", [[order2]])

            await expect(esDex.multicall([makeData1, makeData2], true))
                .to.be.revertedWith("HD: multi value calls")
        })
    })
